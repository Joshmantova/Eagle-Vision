...
if __name__ == '__main__':
    model = Resnet_Model()
    train_data = datasets.ImageFolder('~/Users/josh-mantovani/downloads/archive/train')
    val_data = datasets.ImageFolder('~/Users/josh-mantovani/downloads/archive/valid')
    train_data.transform = model.train_transform
    val_data.transform = model.val_transform
    batch_size = 16

    train_loader = DataLoader(train_data, batch_size, True)
    val_loader = DataLoader(val_data, batch_size, True)

    history, loss, acc = model.fit(train_loader, val_loader)
    predicions, labels = model.evaluate(val_loader)
